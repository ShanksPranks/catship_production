...
theGame.prototype = {
 create: function() {
  ...
  wind = this.game.add.audio('wind1', windVolume, true);
  wind.play('', 0, windVolume, true);
...
var layer;
   ...
   map.addTilesetImage('Terrain1', 'Terrain1');
    
    stage1 = map.createLayer('Terrain1');
  ...
  stage1 = this.game.add.tileSprite(0, 0, 800, 4400, 'stage1');
  stage1.y = -bgh + this.game.world.height;
  stage1.x = (this.game.world.width - bgw) / 2;
  stage1.autoScroll(0, 60);
  ...
  //  The score
  scoreString = 'Score : ';
  scoreText = this.game.add.text(16, 16, scoreString + score, {
   font: '12px Arial',
   fill: '#ef1227'
  });
  ...
  this.game.add.text(this.game.world.width - 100, 10, 'Lives : ' + lives, {
   font: '12px Arial',
   fill: '#ef1227'
  });
...
*/
 ...
 createAliensDynamic: function(level, tweens, alienGroup, enemySpeed) {
  ...
  tweenObj[tweenCount] = this.game.add.tween(alien)
   .to({
    x: catshipLevels.Aliens[level].Tweens[tweens].x,
    y: catshipLevels.Aliens[level].Tweens[tweens].y
   }, catshipLevels.Aliens[level].Tweens[tweens].t, Phaser.Easing.Linear.None)
   .to({
    x: catshipLevels.Aliens[level].Tweens[tweens + 1].x,
    y: catshipLevels.Aliens[level].Tweens[tweens + 1].y
   }, catshipLevels.Aliens[level].Tweens[tweens + 1].t / enemySpeed, Phaser.Easing.Linear.None)
   .to({
    x: catshipLevels.Aliens[level].Tweens[tweens + 2].x,
    y: catshipLevels.Aliens[level].Tweens[tweens + 2].y
   }, catshipLevels.Aliens[level].Tweens[tweens + 2].t / enemySpeed, Phaser.Easing.Linear.None)
   .to({
    x: catshipLevels.Aliens[level].Tweens[tweens + 3].x,
    y: catshipLevels.Aliens[level].Tweens[tweens + 3].y
   }, catshipLevels.Aliens[level].Tweens[tweens + 3].t / enemySpeed, Phaser.Easing.Linear.None)
   .to({
    x: catshipLevels.Aliens[level].Tweens[tweens + 4].x,
    y: catshipLevels.Aliens[level].Tweens[tweens + 4].y
   }, catshipLevels.Aliens[level].Tweens[tweens + 4].t / enemySpeed, Phaser.Easing.Linear.None)
   .to({
    x: catshipLevels.Aliens[level].Tweens[tweens + 5].x,
    y: catshipLevels.Aliens[level].Tweens[tweens + 5].y
   }, catshipLevels.Aliens[level].Tweens[tweens + 5].t / enemySpeed, Phaser.Easing.Linear.None)
   .to({
    x: catshipLevels.Aliens[level].Tweens[tweens + 6].x,
    y: catshipLevels.Aliens[level].Tweens[tweens + 6].y
   }, catshipLevels.Aliens[level].Tweens[tweens + 6].t / enemySpeed, Phaser.Easing.Linear.None)
   .to({
    x: catshipLevels.Aliens[level].Tweens[tweens + 7].x,
    y: catshipLevels.Aliens[level].Tweens[tweens + 7].y
   }, catshipLevels.Aliens[level].Tweens[tweens + 7].t / enemySpeed, Phaser.Easing.Linear.None)
   .loop()
   .start();
  enemyCount += 1;
  tweenCount += 1;
 ...
 updateCounter: function() {
  ...
  if (enemyCount > 16 && enemy4Count == 0 && health >= 0 && sb == 6) {
   ...
   if (debug == 1) {
    console.log(
     'checkpointTotal             = ' + checkpointTotal + 'checkpointEnemyCount        = ' + checkpointEnemyCount + 'checkpointSb                = ' + checkpointSb + 'checkpointPyramidCount      = ' + checkpointPyramidCount + 'checkpointPyramidTimer      = ' + checkpointPyramidTimer + 'checkpointPyramidsOffset    = ' + checkpointPyramidsOffset + 'checkpointStage1Y           = ' + checkpointStage1Y
     // + 'checkpointStage2Y           = ' + checkpointStage2Y
     + 'checkpointPyramidsY         = ' + checkpointPyramidsY + 'checkpointCheckpoint        = ' + checkpointCheckpoint + 'checkpointShipFP            = ' + checkpointShipFP + 'checkpointStagespeed        = ' + checkpointStagespeed
    );
   }
   sb += 1;
   eventTime = total;
