...
gameOver.prototype = {
   ...
   if (!myJson) {
    ...
    if (debug == true) {
     for (var i = 0; i < myJson.players.length; i++) {
      console.log('this was retrieved from the server: \n Name' + myJson.players[i].name + '\n Games Played: ' + myJson.players[i].gamesPlayed + '\n High Score: ' + myJson.players[i].highScore + '\n Last Played Date: ' + myJson.players[i].lastPlayedDate + '\n catshipID: ' + myJson.players[i].catshipID);
     }
    }
    ...
    var stateText = this.game.add.text(this.game.world.height / 2, this.game.world.width / 2 + 30, score, {
     fontSize: '50px',
     fill: '#ef1227'
    });
    ...
    $.ajax({
     ...
     data: {
      jsonObject: JSON.stringify(myJson)
     },
     success: function(data) {
 ...
 playTheGame: function() {
  ...
  if (lives == 0 && storyboard == 5) {
   ...
   bg.tint = 0x221804;
   var titleText = this.game.add.text((this.game.world.width / 2) - 130, 40, 'BEST ' + myJson.players.length + ' PLAYERS', {
    fontSize: '30px',
    fill: '#efed30'
   });
   var gamesText = this.game.add.text((this.game.world.width / 2), this.game.world.height - 40, 'GLOBAL CATSHIP FLIGHTS ' + gamesPlayed, {
    fontSize: 10,
    fill: '#efed30'
   });
   ...
   for (var i = myJson.players.length - 1; i >= 0; i--) {
    var newText = this.game.add.text(60, 80 + (offSet - i + 1) * 40, (offSet - i + 1) + '   ', {
     fontSize: '30px',
     fill: '#f1e6e3'
    });
    var nextText = this.game.add.text(100, 80 + (offSet - i + 1) * 40, myJson.players[i].highScore + '   ', {
     fontSize: '30px',
     fill: '#56d7cf'
    });
    var finalText = this.game.add.text(200, 80 + (offSet - i + 1) * 40, myJson.players[i].name, {
     fontSize: '30px',
     fill: '#e00a0a'
    });
